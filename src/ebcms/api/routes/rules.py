...
from ebcms.models import AuditLog, BrokerageRule
...
def create_rule(payload: RuleCreate, db: Session = Depends(get_db)) -> BrokerageRule:
...
def list_rules(active_only: bool = True, db: Session = Depends(get_db)) -> list[BrokerageRule]:
...
def update_rule(rule_id: int, payload: RuleUpdate, db: Session = Depends(get_db)) -> BrokerageRule:
...
def deactivate_rule(rule_id: int, db: Session = Depends(get_db)) -> BrokerageRule:
    rule = db.get(BrokerageRule, rule_id)
    if not rule:
        raise HTTPException(status_code=404, detail="Rule not found.")
    old_value = _rule_snapshot(rule)
